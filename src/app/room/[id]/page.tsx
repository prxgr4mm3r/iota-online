...
.on('postgres_changes', { event:'*', schema:'public', table:'states', filter:`room_id=eq.${id}` }, (payload)=>{
...
function onPick(c: Card){ setPicked(c); }
...
newGrid[k] = picked;
await supabase.from('states').update({ grid: newGrid, version: (state?.version||0)+1 }).eq('room_id', id);
setPicked(null);
...
{picked && <div className="text-sm text-gray-500">Picked: {picked.kind==='wild'?'★':`${picked.color}/${picked.shape}/${picked.num}`}</div>}
</div>
</main>
);
}